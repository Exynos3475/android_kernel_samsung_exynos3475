...
int exynos_ss_save_reg(void *v_regs)
{
	...
	if (!regs) {
		asm("str r0, [%0, #0]\n\t"
		    ...
		    :: "r"(core_reg));
		core_reg->ARM_pc =
			(unsigned long)(core_reg->ARM_lr - sizeof(unsigned int));
	} else {
...
static void exynos_ss_dump_one_task_info(struct task_struct *tsk, bool is_main)
{
	...
	if (lookup_symbol_name(wchan, symname) < 0) {
		if (!ptrace_may_access(tsk, PTRACE_MODE_READ))
			...
			snprintf(symname, KSYM_NAME_LEN, "%zu", wchan);
	}
	...
	pr_info("%8d %8d %8d %16lld %c(%d) %3d  %16zx %16zx  %16zx %c %16s [%s]\n",
			...
			task_cpu(tsk), wchan, pc, (size_t)tsk,
...
void exynos_ss_task(int cpu, void *v_task)
{
	...
	if (unlikely(!ess_base.enabled || !item->entry.enabled))
		return;
	{
		unsigned i = atomic_inc_return(&ess_idx.task_log_idx[cpu]) &
...
void exynos_ss_cpuidle(int index, unsigned state, int diff, int en)
{
	...
	{
		int cpu = raw_smp_processor_id();
		unsigned i = atomic_inc_return(&ess_idx.cpuidle_log_idx[cpu]) &
...
void exynos_ss_suspend(void *fn, void *dev, int en)
{
	...
	{
		int cpu = raw_smp_processor_id();
		unsigned i = atomic_inc_return(&ess_idx.suspend_log_idx[cpu]) &
...
void exynos_ss_mailbox(void *msg, int mode, char* f_name, void *volt)
{
	...
	{
		int cpu = raw_smp_processor_id();
		unsigned i = atomic_inc_return(&ess_idx.mailbox_log_idx) &
...
#endif /* CONFIG_ARM64 */
#endif /* CONFIG_EXYNOS_SNAPSHOT_IRQ_DISABLED */
...
void exynos_ss_spinlock(void *v_lock, int en)
{
		...
		return;
	{
		...
		unsigned j, i = index & (ARRAY_SIZE(ess_log->spinlock[0]) - 1);
...
void exynos_ss_irqs_disabled(unsigned long flags)
{
	...
	if (unlikely(flags)) {
		...
		unsigned j, i = index % ARRAY_SIZE(ess_log->irqs_disabled[0]);
...
void exynos_ss_clk(struct clk *clk, int en)
{
	...
	{
		int cpu = raw_smp_processor_id();
		unsigned i = atomic_inc_return(&ess_idx.clk_log_idx[cpu]) &
...
void exynos_ss_freq(int type, unsigned long freq, int en)
{
	...
	{
		int cpu = raw_smp_processor_id();
		unsigned i = atomic_inc_return(&ess_idx.freq_log_idx) &
...
void exynos_ss_hrtimer(void *timer, s64 *now, void *fn, int en)
{
	...
	{
		int cpu = raw_smp_processor_id();
		unsigned i = atomic_inc_return(&ess_idx.hrtimer_log_idx[cpu]) &
