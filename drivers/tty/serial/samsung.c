...
{
	void __iomem *change_reg0, *change_reg1;
	u32 temp_val0,  temp_val1, cmd;
	int ret;

	printk("Change UART port to CP\n");
	change_reg0 = ioremap(0x139c0000, SZ_4);
	change_reg1 = ioremap(0x139b0000, SZ_4);

	ret = sscanf(buf, "%d", &cmd);

	switch (cmd) {
	case 0:
		printk("Change UART to CP\n");
		temp_val0 = readl(change_reg0 + 0x40);
		temp_val0 |= (0x3 << 4) | 0x3;
		writel(temp_val0, change_reg0 + 0x40);
		break;
	case 1:
		printk("Change T32 to CP\n");
		temp_val0 = readl(change_reg0 + 0xa0);
		temp_val0 |= (0x3 << 24) | (0x3 << 20) | (0x3 << 16);
		writel(temp_val0, change_reg0 + 0xa0);

		temp_val0 = readl(change_reg0 + 0xc0);
		temp_val0 |= 0x3;
		writel(temp_val0, change_reg0 + 0xc0);

		temp_val1 = readl(change_reg1 + 0x20);
		temp_val1 |= (0x2 << 28);
		writel(temp_val1, change_reg1 + 0x20);
		break;
	case 2:
		printk("Change UART/T32 to CP\n");
		temp_val0 = readl(change_reg0 + 0x40);
		temp_val0 |= (0x3 << 4) | 0x3;
		writel(temp_val0, change_reg0 + 0x40);
...
static DEVICE_ATTR(uart_change, 0640, uart_change_show, uart_change_store);
...
static int s3c24xx_serial_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(&pdev->dev, &dev_attr_uart_change);
	if (ret < 0)
